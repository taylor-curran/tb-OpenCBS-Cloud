...
export class LoanDetailsFormComponent implements OnInit {
  ...
  @Output() creditLineSelectChange = new EventEmitter();
  @Output() payeeEditChange = new EventEmitter();
  @Output() payeeDeleteChange = new EventEmitter();
  @Output() payeeAddChange = new EventEmitter();
  @Output() entryFeesClickChange = new EventEmitter();
    ...
    if ( creditLine ) {
      this.creditLinesData.forEach(val => {
        if ( val.id === creditLine ) {
          ...
          this.creditLineSelectChange.emit(val);
        }
      });
    } else {
  ...
  onLPSelect(loanProduct, creditLine?) {
    if ( loanProduct ) {
      ...
      this.loanProductSelectChange.emit(loanProduct);
      ...
      this.loanProductSelectChange.emit(null);
    }
