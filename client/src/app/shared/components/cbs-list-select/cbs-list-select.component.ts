...
export class ListSelectComponent {
  ...
  @Output() selectItemChanged = new EventEmitter();
  @Output() removeItemChanged = new EventEmitter();


  public pick: any = [];
  public open = false;
  selectItem(pickedItem) {
    this.selectItemChanged.emit(pickedItem);
  }

  delete(removedItem) {
    this.removeItemChanged.emit(removedItem);
  }
}
