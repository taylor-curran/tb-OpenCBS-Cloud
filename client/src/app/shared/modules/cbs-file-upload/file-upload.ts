...
export class FileUploadComponent implements OnInit, AfterContentInit {
  ...
  @Output() customError: EventEmitter<any> = new EventEmitter();

  @Output() clear: EventEmitter<any> = new EventEmitter();

  @Output() customSelect: EventEmitter<any> = new EventEmitter();
  ...
  onFileSelect(event) {
    ...
    this.customSelect.emit({originalEvent: event, files: files});
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState === 4) {
        ...
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          this.customError.emit({xhr: xhr, files: this.files});
        }
        this.clear();
