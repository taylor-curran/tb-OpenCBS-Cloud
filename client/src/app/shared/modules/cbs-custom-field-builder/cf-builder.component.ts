...
export class CFBuilderComponent {
  ...
  @Input() fieldTypes = [];
  @Output() onSectieditSuccess = new EventEmitter();
  @Output() onSectieditError = new EventEmitter();
  ...
  callSectieditSuccess(editedSectionData) {
    this.onSectieditSuccess.emit(editedSectionData);
  }

  callSectieditError(error) {
    this.onSectieditError.emit(error);
  }
