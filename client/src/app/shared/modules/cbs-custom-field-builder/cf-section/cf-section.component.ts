...
export class CFSectionComponent implements OnInit {
  ...
  @Output() editChangeSuccess = new EventEmitter();
  @Output() editChangeError = new EventEmitter();
  @Output() addChangeSuccess = new EventEmitter();
  @Output() addChangeError = new EventEmitter();
  @Output() addChangeCancel = new EventEmitter();
  ...
  updateSection(data) {
    this.service.updateSection(this.url, this.sectionData.id, data).subscribe(
      resp => {
        this.editChangeSuccess.emit(resp);
        this.closeEdit();
      },
      err => {
        alert(err.error.message);
        this.editChangeError.emit(err.error);
      }
    );
  }
  ...
  createSection(data) {
    this.service.createSection(this.url, data).subscribe(
      resp => {
        this.addChangeSuccess.emit(resp);
        this.closeEdit();
        this.newSectionMode = false;
      },
      err => {
        alert(err.error.message);
        this.addChangeError.emit(err.error);
      }
    );
  }
