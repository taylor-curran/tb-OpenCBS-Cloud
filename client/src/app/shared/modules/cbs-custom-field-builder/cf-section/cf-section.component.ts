...
export class CFSectionComponent {
  ...
  @Output() editSuccessChanged = new EventEmitter();
  @Output() editErrorChanged = new EventEmitter();
  @Output() addSuccessChanged = new EventEmitter();
  @Output() addErrorChanged = new EventEmitter();
  @Output() addCancelChanged = new EventEmitter();
  ...
  updateSection(data) {
    this.service.updateSection(this.url, this.sectionData.id, data).subscribe(
      resp => {
        this.editSuccessChanged.emit(resp);
        this.closeEdit();
      },
      err => {
        alert(err.error.message);
        this.editErrorChanged.emit(err.error);
      }
    );
  }
  ...
  createSection(data) {
    this.service.createSection(this.url, data).subscribe(
      resp => {
        this.addSuccessChanged.emit(resp);
        this.closeEdit();
        this.newSectionMode = false;
      },
      err => {
        alert(err.error.message);
        this.addErrorChanged.emit(err.error);
      }
    );
  }
