...
export class CFFieldComponent implements OnInit, AfterViewInit {
  ...
  @Output() fieldAddCancelChanged = new EventEmitter();
  ...
  cancel() {
    ...
    if ( this.newFieldMode ) {
      this.fieldAddCancelChanged.emit(this.fieldData.sectionId);
    }
    this.formCollapsed = true;
  }
  ...
  updateField(data: Field) {
    this.service.updateField(this.url, this.fieldData.id, data).subscribe(
      resp => {
        this.fieldEditSuccessChanged.emit(resp);
        this.cancel();
      },
      err => {
        this.fieldEditErrorChanged.emit(err.error);
      }
    );
  }
  ...
  addNewField(data: Field) {
    this.service.createField(this.url, data).subscribe(
      resp => {
        this.fieldAddSuccessChanged.emit({data: resp, sectionId: data.sectionId});
        this.newFieldMode = false;
        this.formChanged = false;
        this.cancel();
      },
      err => {
        this.fieldAddErrorChanged.emit(err.error);
      }
    );
  }
      ...
      () => {
        ...
        if ( this.url && this.fieldData.id ) {
          ...
          this.service.getFields(urlData).subscribe(
            res => {
              this.fieldDeleteSuccessChanged.emit(res);
              this.cancel();
            });
        }
      },
      err => {
        this.fieldDeleteErrorChanged.emit(err.error);
      }
    );
  }
