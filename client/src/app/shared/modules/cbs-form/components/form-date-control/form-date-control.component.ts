...
export class FormDateControlComponent implements OnInit, ControlValueAccessor, OnChanges {
  ...
  set value(v: any) {
    if ( v && v.value && v.value.isValid() ) {
      if ( !v.value.isSame(this.innerValue) ) {
        ...
        this.changeChangedCallback(v.value.format(this.localStorageService.getDateFormat()));
        this.changeChanged.emit(v.value.format(this.localStorageService.getDateFormat()));
      }
    }
  }
  ...
  ngOnChanges(inputs) {
    if ( inputs.validateDate && this.innerValue ) {
      ...
      this.changeChangedCallback(this.innerValue.format(this.systemSettingsShareService.getData('DATE_FORMAT')));
    }
  }
