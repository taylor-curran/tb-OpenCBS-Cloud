...
export class PicklistComponent implements OnInit, OnChanges {
  ...
  @Output() clearChanged = new EventEmitter();
  @Output() picklistOpenChanged = new EventEmitter();
  @Output() picklistCloseChanged = new EventEmitter();
  ...
  select(item) {
    ...
    this.isOpened = false;
    this.picklistCloseChanged.emit();
    this.valueString = item[this.filterType];
  }
